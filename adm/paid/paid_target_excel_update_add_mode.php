...
if(empty($batch_id)) {
    alert_close('잘못된 요청입니다.(배치)');
}

// (선택) gnuboard admin token 유지
$chk = check_admin_token();
if (!$chk) {
    alert_close('잘못된 요청입니다.(토큰)');
}
...
try {
    // $file = 업로드된 tmp 파일 경로
    // 원본 파일명에서 확장자 가져오기 (키는 실제 input name으로 바꿔줘)
    ...
    if ($reader instanceof CsvReader) {
        ...
        // PhpSpreadsheet CSV는 input encoding을 setInputEncoding으로 설정
        if (!mb_check_encoding($csvRaw, 'UTF-8')) {
            $reader->setInputEncoding('CP949'); // 또는 'EUC-KR'
        } else {
        ...
        $reader->setDelimiter(',');
        $reader->setEnclosure('"');
        // CsvReader에는 setSheetIndex가 없고, 로드 후 시트(0) 접근하면 됨
    }

    $reader->setReadDataOnly(true);

    // canRead 체크(여기서 걸리면 "확장자만 엑셀"인 가짜 파일 가능성 높음)
    if (method_exists($reader, 'canRead') && !$reader->canRead($file)) {
        alert_close('엑셀 파일 형식을 확인할 수 없습니다. 파일이 손상되었거나 실제 엑셀이 아닐 수 있습니다.');
    }

    // 로드
    $objPHPExcel = $reader->load($file); // (변수명 유지)

    $sheetCount = $objPHPExcel->getSheetCount();
    if ($sheetCount <= 0) {
        alert_close('엑셀 시트를 읽을 수 없습니다. 비밀번호 또는 파일 형식을 확인하세요.');
    }
...
if ($is_headerless) {
    ...
    $idx_name = $idx_hp = $idx_birth = $idx_sex = $idx_user_key = null;

    $header_lc = array_map('mb_strtolower', $header);
    $name_lc   = array_map('mb_strtolower', $name_keys);
    $hp_lc     = array_map('mb_strtolower', $hp_keys);
    $birth_lc  = array_map('mb_strtolower', $birth_keys);
    $sex_lc    = array_map('mb_strtolower', $sex_keys);
    $user_key_lc    = array_map('mb_strtolower', $user_keys);

    foreach ($header_lc as $i => $h) {
        if ($idx_name  === null && in_array($h, $name_lc,  true))  $idx_name  = $i;
        if ($idx_hp    === null && in_array($h, $hp_lc,    true))  $idx_hp    = $i;
        if ($idx_birth === null && in_array($h, $birth_lc, true))  $idx_birth = $i;
        if ($idx_sex   === null && in_array($h, $sex_lc,   true))  $idx_sex   = $i;
        if ($idx_user_key   === null && in_array($h, $user_key_lc,true))  $idx_user_key = $i;
    }
...
try {
    for ($i = $start_row; $i <= $num_rows; $i++) {
        ...
        $user_key = 0;
        if(!$is_headerless) {
            if ($idx_user_key !== null && isset($row[$idx_user_key])) $user_key = (int)$row[$idx_user_key];
        }
        ...
        if ($is_headerless) {
            ...
            $meta = [];
            foreach ($header as $k=>$h) {
                if ($k === $idx_hp || $k === $idx_name || $k === $idx_birth || $k === $idx_sex || $k == $idx_user_key) continue;
        ...
        $sql = "INSERT INTO add_mode_call_stg_target_upload
                (batch_id, rand_score, user_key, call_hp, name,
                 birth_date, db_age_type, sex, meta_json, created_at)
                VALUES
                ('{$batch_id}', '{$rand_score}', '{$user_key}', '{$call_hp}', ".sql_quote_or_null($name).",
